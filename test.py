...
def create_gameroom():
    ...
    gameRoom = CreateGameRoomDto(worldAmount=world_id, playerAmount=world_id + 1, gameRoomName=game_room_name,
                                 gameRoomPassword=room_password)
    global game_room
    game_room = game_room_name
    response = httpSession.post(f"http:{target_url}/rest/gameroom", json=gameRoom.as_dict(), verify=False)
    if response.status_code >= 300:
        print(response.json())
        raise RuntimeError("Failed To Create a GameRoom!")

def create_player(player: PlayerDto):
    response = httpSession.post(f"http:{target_url}/rest/gameroom/{game_room}?password={room_password}",
                                json=player.as_dict(), verify=False)
    print(response)
    if response.status_code >= 300:
        print(response.json())
...
async def test():
    ...
    async with websockets.connect(f"ws:{target_url}/ws", extra_headers={"sessionId": None}) as websocket:
        ...
        await websocket.send(f.subscribe(f"/topic/coop/{game_room}", room_password))
        print("Subscribed")
        # foo = await websocket.recv()
        # print(foo)
        # test_dto = ItemDto(world_id, 10, 0x34)
        test_dto = CoopItemDto("Bar", 0x11)
        # test_dto = CoopDto("Foobar", 0x34)
        await websocket.send(f.send_json(f"/app/coop/{game_room}", json.dumps(test_dto.as_dict()), room_password))
